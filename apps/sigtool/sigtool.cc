...
static int GenSample(const std::string& out_dir, const std::string& version) {
  ...
  if (!WriteFile(dex_path, dex)) {
    std::fprintf(stderr, "sigtool: failed to write %s\n",
  ...
  std::fprintf(stdout, "wrote %s (%zu bytes)\n", sig_path.string().c_str(),
               sig.size());
  std::fprintf(stdout, "try: aavscan %s %s\n", sig_path.string().c_str(),
               dex_path.string().c_str());
  return 0;
}
...
int main(int argc, char** argv) {
  if (argc >= 2 && std::string(argv[1]) == "gen-sample") {
    ...
    if (version.size() != 3) {
      std::fprintf(stderr,
                   "sigtool: --dex-version must be 3 digits, e.g. 039\n");
      return 2;
    }
    return GenSample(out_dir, version);
  }
  std::fprintf(stderr, "usage: %s gen-sample <out_dir> [--dex-version NNN]\n",
               argv[0]);
  return 2;
}
