...
#include "dex/dex_sig.h"           // STR/LOGIC match types, kBitMapSize
#include "sig/malware_name.h"      // MalwareName / MalwareFamily / SigName
#include "sig/sig_db_structure.h"  // SigHeader / SigSectionHeader / kSigMagic
#include "utils/blowfish.h"        // Blowfish
#include "utils/crc32.h"           // aav::Crc32
...
const char kBlowfishKey[] = "aav-dex-malware-signature-db-v1";
const uint32_t kNameId = 1;
const uint32_t kFamilyId = 1;
...
const char kEvilType[] = "Lcom/aav/sample/Evil;";
const char kWorkerType[] = "Lcom/aav/sample/Worker;";
const char kConstString[] = "evilpayload";

// Opcode buffer DexCode::ParseCode produces for Worker.work().
const uint8_t kOpcodeBuf[] = {0x12, 0x12, 0x12, 0x1a, 0x00, 0x00, 0x00, 0x0e};
const uint16_t kFast01 = 0x1212;
const uint16_t kFast23 = 0x1a12;
const uint16_t kFast45 = 0x0000;
const uint16_t kFast67 = 0x0e00;
...
Bytes BuildSampleDex(const std::string& version) {
  ...
  for (uint32_t i = 0; i < k_num_strings; ++i) {
    ...
    PutUleb(d, static_cast<uint32_t>(std::strlen(strings[i])));
    PutStrZ(d, strings[i]);
  }
...
Bytes BuildSampleSig() {
  ...
  Bytes operand;
  PutBytes(operand, kConstString, std::strlen(kConstString));
  operand.push_back(0);
