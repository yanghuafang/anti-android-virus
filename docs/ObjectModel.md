...
- Engine objects are **neither copyable nor movable**, and that is stated once:
  `IObject` deletes its copy constructor and copy assignment, so every
  interface and every implementation below inherits the deletion and a slicing
  copy is a compile error rather than a convention. Classes outside the
  hierarchy — `DexFile`, `DexParser`, the signature managers, the scan-result
  accumulators — each declare their own deletions with the reason.
- Every concrete implementation is **`final`**. Nothing in the tree derives
  from one, they are reachable only through their factory, and `final` is what
  makes the `Init()` / `Uninit()` call in each destructor a non-virtual call —
  the Google style rule about doing work in constructors and destructors.
