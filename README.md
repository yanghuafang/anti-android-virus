...
| `analysis.yml`    | the gcovr line-coverage floor; the fuzz harness builds on every change, the fuzzer itself runs nightly |
...
`Analysis / Fuzz run (15m)` and `Docs / deploy` are the two that do not gate a
pull request: the first runs nightly, the second only publishes from `main`.

The fuzzer is the one thing that does not gate a pull request: a seedless run
explores different input every time, so a finding there is an issue to open
rather than an unrelated pull request to fail. What *does* gate is that the
harness still compiles; the run happens nightly and uploads any crashing input.
