...
MemStream::MemStream() : mode_(0), buf_(nullptr), buf_size_(0), pos_(0) {}
...
int MemStream::Init(MemSource* source) {
  ...
  if (nullptr == source->buf) {
    return -1;
  }
  buf_ = static_cast<char*>(source->buf);
  if (0 == source->buf_size) {
    return -1;
  }
...
int MemStream::GetFullPath(char* /*path_buf*/, int /*path_buf_size*/) {
  return 0;
}
...
int MemStream::GetView(const void** view, int64_t* size) {
  if (nullptr == view || nullptr == size || nullptr == buf_) {
    return -1;
  }
