...
FileStream::FileStream() : mode_(0), file_(nullptr), file_map_(nullptr) {}
...
int FileStream::Init(FileSource* source) {
  ...
  try {
    mode_ = source->mode;
    if (O_RDONLY != mode_ && O_WRONLY != mode_ && O_RDWR != mode_) {
      return -1;
    }

    if (nullptr != source->name) {
      name_ = source->name;
    }

    if (nullptr == source->path) {
      return -1;
    }
    path_ = source->path;
...
int FileStream::GetSize(int64_t* size) {
  ...
  if (0 != std::fseek(file_, 0, SEEK_END)) {
    return -1;
  }
  *size = std::ftell(file_);

  if (0 != std::fseek(file_, pos, SEEK_SET)) {
...
int FileStream::GetName(char* name_buf, int name_buf_size) {
  if (nullptr == name_buf || name_buf_size < 1) {
    return -1;
  }

  std::memset(name_buf, 0, name_buf_size);
  std::strncpy(name_buf, name_.c_str(), name_buf_size - 1);
  return 0;
}

int FileStream::GetFullPath(char* path_buf, int path_buf_size) {
  if (nullptr == path_buf || path_buf_size < 1) {
    return -1;
  }

  std::memset(path_buf, 0, path_buf_size);
  std::strncpy(path_buf, path_.c_str(), path_buf_size - 1);
  return 0;
}
...
int FileStream::Read(void* buf, int bytes_to_read, int* bytes_read) {
  ...
  *bytes_read = std::fread(buf, 1, bytes_to_read, file_);
  if (0 == *bytes_read) {
    return -1;
  }
  return 0;
...
int FileStream::Write(void* buf, int bytes_to_write, int* bytes_written) {
  ...
  *bytes_written = std::fwrite(buf, 1, bytes_to_write, file_);
  if (0 == *bytes_written) {
    return -1;
  }
  return 0;
...
int FileStream::Seek(int64_t offset, int method) {
  if (SEEK_SET != method && SEEK_CUR != method && SEEK_END != method) {
    return -1;
  }

  if (0 == std::fseek(file_, offset, method)) {
    return 0;
  }
  return -1;
}
