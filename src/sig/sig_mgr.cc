...
#include "sig/common_define.h"
#include "sig/malware_name.h"
#include "sig/sig_db_structure.h"
#include "utils/blowfish.h"
#include "utils/file_uncompress.h"
#include "utils/log.h"
...
#define AAV_RETURN_ERROR(msg, ret)      \
  do {                                  \
...
int SigMgr::Init(void* /*context*/) {
  ...
  file_buffer_len_ = 0;
  std::memset(&sig_header_, 0, sizeof(SigHeader));
...
int SigMgr::CheckAndLoadSigs(const char* path,
  ...
  if (decrypted.empty()) {
    AAV_RETURN_ERROR("Blowfish", kAdSigEncrypt);
  }

  // uncompress
  std::string uncompress_data;
  if (!GzipInflate(decrypted, uncompress_data)) {
    AAV_RETURN_ERROR("uncompress", kAdSigUncompress);
  }
  ...
  file_buffer_ptr_ = static_cast<unsigned char*>(std::malloc(file_buffer_len_));
  if (nullptr == file_buffer_ptr_) {
    AAV_RETURN_ERROR("malloc", kAdSigLoad);
  }
  std::memset(file_buffer_ptr_, '\0', file_buffer_len_);
  std::memcpy(file_buffer_ptr_, uncompress_data.c_str(), file_buffer_len_);

  // get sig file header
  std::memcpy(&tmp_sig_header, file_buffer_ptr_, sig_header_len);
  AAV_LOGD("sig DB magic=0x%x version=%u", tmp_sig_header.magic,
           tmp_sig_header.version);
  if (tmp_sig_header.magic != kSigMagic) {
    AAV_RETURN_ERROR("magic", kAdSigMagic);
  }
  ...
  if (tmp_sig_header.crc != crc) {
    AAV_RETURN_ERROR("crc", kAdSigCrc);
  }

  if (updated && tmp_sig_header.version <= sig_header_.version) {
    AAV_RETURN_ERROR("version", kAdSigLowVersion);
  }
  ...
  while (offset < file_buffer_len_) {
    ...
    if (file_buffer_len_ - offset < sig_section_header_len) {
      AAV_RETURN_ERROR("section header truncated", kAdSigLoad);
    }
    SigSectionHeader section_header;
    std::memcpy(&section_header, file_buffer_ptr_ + offset,
                sig_section_header_len);
    ...
    if (section_header.packed_size >
      ...
      AAV_RETURN_ERROR("section payload truncated", kAdSigLoad);
    }

    SigItem* sig_item = static_cast<SigItem*>(std::malloc(sizeof(SigItem)));
    if (nullptr == sig_item) {
      AAV_RETURN_ERROR("malloc", kAdSigLoad);
    }
    std::memset(sig_item, '\0', sizeof(SigItem));
    ...
    if (kAdSuccess != DealWithSection(sig_item)) {
      AAV_RETURN_ERROR("section parse", kAdSigLoad);
    }
    offset += sig_item->buf_size;
  }

  int path_len = std::strlen(path);
  sig_file_path_ = static_cast<char*>(std::malloc(path_len + 1));
  std::memset(sig_file_path_, '\0', path_len + 1);
  std::strncpy(sig_file_path_, path, path_len);
  std::memcpy(&sig_header_, &tmp_sig_header, sig_header_len);
  return kAdSuccess;
}
...
int SigMgr::GetAdInfo(int sig_id, void** ad_info) {
  *ad_info = nullptr;
  for (std::vector<AdInfoNode>::iterator it = ad_info_vector_.begin();
    ...
    if ((it)->sig_id == static_cast<uint32_t>(sig_id)) {
      *ad_info = (it)->ad_info;
      return kAdSuccess;
    }
  }
...
int SigMgr::GetMalwareName(int sig_id, char* name_buf, int name_buf_size) {
  if (nullptr == name_buf || name_buf_size <= 0) {
    return kAdError;
  }
  ...
  int name_id = -1;
  {
    ...
    while (index + static_cast<int>(sizeof(SigName)) <= bufsz) {
      SigName sig_name;
      std::memcpy(&sig_name, base + index, sizeof(sig_name));
  ...
  while (index + static_cast<int>(sizeof(MalwareName)) <= bufsz) {
    ...
    std::memcpy(&malware_name, base + index, sizeof(malware_name));
    if (malware_name.id == static_cast<uint32_t>(name_id)) {
      ...
      std::memset(name_buf, '\0', name_buf_size);
      std::snprintf(name_buf, name_buf_size, "%s!%s.%s@%s.%s",
...
static int BoundedCStr(const char* base, int index, int bufsz,
  ...
  if (len == maxlen) {
    return -1;  // no terminator inside the section
  }
  if (out != nullptr) {
    out->assign(p, len);
  }
...
int SigMgr::BuildDataAdinfo(SigItem* sig_item) {
  ...
  while (index < bufsz) {
    ...
    std::memset(&ad_info_node, '\0', sizeof(AdInfoNode));
    ad_info_node.ad_info = base + index;

    // sig id (u32)
    if (bufsz - index < static_cast<int>(sizeof(uint32_t))) {
      return kAdError;
    }
    uint32_t sig_id = 0;
    std::memcpy(&sig_id, base + index, sizeof(sig_id));
...
int SigMgr::BuildDataFamily(SigItem* sig_item) {
  ...
  while (index < bufsz) {
    ...
    uint32_t id = 0;
    std::memcpy(&id, base + index, sizeof(id));
