...
constexpr uint8_t kDexMagicPrefix[4] = {0x64, 0x65, 0x78, 0x0a};  // "dex\n"

bool IsDexMagic(const void* buf) {
  const uint8_t* p = static_cast<const uint8_t*>(buf);
  if (0 != std::memcmp(p, kDexMagicPrefix, sizeof(kDexMagicPrefix))) {
    return false;
  }
  if (0x00 != p[7]) {
    return false;
...
int FileId::GetFileType(IStream* stream, FileType* file_type) {
  if (nullptr == stream || nullptr == file_type) {
    return -1;
  }

  int64_t file_size = 0;
  if (0 != stream->GetSize(&file_size)) {
    return -1;
  }
  if (file_size < 16) {
    return -1;
  ...
  char buf[16] = {0};
  int bytes_read = 0;
  if (0 != stream->Read(buf, sizeof(buf), &bytes_read)) {
    return -1;
  }
  ...
  if (0 == std::memcmp(buf, zip_file_magic, sizeof(zip_file_magic))) {
...
int FileId::GetFileType(ITarget* target, FileType* file_type) {
  if (nullptr == target || nullptr == file_type) {
    return -1;
  }

  int64_t file_size = 0;
  if (0 != target->GetSize(&file_size)) {
    return -1;
  }
  if (file_size < 16) {
    return -1;
...
int FileId::GetPackType(IStream* stream, PackType* pack_type) {
  if (nullptr == stream || nullptr == pack_type) {
    return -1;
  }
...
int FileId::GetPackType(ITarget* target, PackType* pack_type) {
  if (nullptr == target || nullptr == pack_type) {
    return -1;
  }
  *pack_type = kPackTypeUnknown;
  return 0;
