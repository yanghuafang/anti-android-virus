...
int ApkScanner::ScanApkBytes(const void* data, size_t size,
  ...
  result.reset();
  if (nullptr == data || 0 == size || !dex_scanner_) {
    return -1;
  }
  ...
  for (int i = 1;; ++i) {
    char entry[32];
    if (1 == i) {
      std::snprintf(entry, sizeof(entry), "classes.dex");
    } else {
      std::snprintf(entry, sizeof(entry), "classes%d.dex", i);
    }

    int index = mz_zip_reader_locate_file(&zip, entry, nullptr, 0);
    if (index < 0) {
      break;  // no more multidex members
    }
  ...
  const size_t bytes = n > 0 ? sizeof(ScanResult) + ((n - 1) * sizeof(uint32_t))
                             : sizeof(ScanResult);
  ScanResult* merged = static_cast<ScanResult*>(std::malloc(bytes));
  if (nullptr == merged) {
    return -1;
  }
...
int ApkScanner::ScanStream(IStream* stream, const ScanOption* option,
  ...
  if (nullptr == stream || nullptr == option) {
    return -1;
  }
  result.reset();
  ...
  int64_t size = 0;
  if (0 != stream->GetSize(&size) || size <= 0) {
    return -1;
  }
  std::string buf;
  buf.resize(static_cast<size_t>(size));
  if (0 != stream->Seek(0, 0)) {
    return -1;  // SEEK_SET
  }
  int64_t total = 0;
  while (total < size) {
    ...
    int got = 0;
    if (0 != stream->Read(&buf[total], want, &got) || got <= 0) {
      return -1;
    }
    total += got;
  }
