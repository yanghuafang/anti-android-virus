...
void Blowfish::SetKey(const char* key, size_t byte_length) {
  ...
  if (key == nullptr || byte_length == 0) return;
...
void Blowfish::Encrypt(char* dst, const char* src, size_t byte_length) const {
  ...
  for (size_t i = 0; i < byte_length / sizeof(uint64_t); ++i) {
    uint32_t left, right;
    memcpy(&left, dst + i * 8, sizeof(left));
    memcpy(&right, dst + i * 8 + 4, sizeof(right));
    EncryptBlock(&left, &right);
    memcpy(dst + i * 8, &left, sizeof(left));
    memcpy(dst + i * 8 + 4, &right, sizeof(right));
  }
}
