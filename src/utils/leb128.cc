  ...
  if (v <= 0x7f) {
    ...
    if (cur <= 0x7f) {
      ...
      if (cur <= 0x7f) {
        ...
        v |= (cur & 0x7f) << 21;
        if (cur <= 0x7f) {
          v |= static_cast<int32_t>(static_cast<uint32_t>(v) << 4) >> 4;
        } else {
          cur = *(p++);
          // Shifted as unsigned: the 5th group lands in the top 4 bits, and
          // (cur & 0x7f) << 28 overflows int for any group above 7. Same shape
          // as ReadUleb128Safe below, which had it right already.
          v |= static_cast<int32_t>(static_cast<uint32_t>(cur & 0x7f) << 28);
          if (cur > 0x7f) {
            ret = -1;  // 6th byte needed -> overlong / overflow
          }
        }
      }
    }
  ...
  if (v > 0x7f) {
    ...
    if (cur > 0x7f) {
      ...
      if (cur > 0x7f) {
        ...
        if (cur > 0x7f) {
          cur = *(p++);
          v |= static_cast<uint32_t>(cur & 0x7f) << 28;
          if (cur > 0x7f) {
            ret = -1;
            // assert(false);
          }
