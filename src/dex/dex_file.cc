...
// DEX_FILE_MAGIC and ENDIAN_CONSTANT as the format defines them.
constexpr uint8_t kDexFileMagic[8] = {0x64, 0x65, 0x78, 0x0a,
                                      0x30, 0x33, 0x35, 0x00};  //"dex\n035\0"
constexpr uint32_t kEndianConstant = 0x12345678;
constexpr uint32_t kReverseEndianConstant = 0x78563412;
...
bool IsSupportedDexMagic(const uint8_t* magic) {
  if (0 != std::memcmp(magic, kDexFileMagic, 4)) {
    return false;  // "dex\n"
  }
  if (0x00 != magic[7]) {
    return false;
  ...
  if ('3' == magic[5] && magic[6] >= '5' && magic[6] <= '9') {
    return true;
  }
  if ('4' == magic[5] && '0' == magic[6]) {
    return true;
  }
  return false;
}
...
int DexFile::GetClass(std::string& class_name) {
  ...
  int ret = -1;
  do {
    if (dex_format_->class_defs.end() == class_defs_itor_) {
      break;
    }
    ...
    cur += bytes_read;
    if (0 !=
        ReadUleb128Safe(cur, BufEnd(), &instance_fields_size, &bytes_read)) {
      break;
    }
    cur += bytes_read;
    if (0 !=
        ReadUleb128Safe(cur, BufEnd(), &direct_methods_size, &bytes_read)) {
      break;
    }
    cur += bytes_read;
    if (0 !=
        ReadUleb128Safe(cur, BufEnd(), &virtual_methods_size, &bytes_read)) {
      break;
    }
    cur += bytes_read;
...
int DexFile::GetDirectMethod(std::string& method_name, std::string& proto_name,
  ...
  int ret = -1;
  do {
    if (class_data_item_->direct_methods.end() == direct_methods_itor_) {
      break;
    }
    ...
    dex_code.Uninit();
    if (0 != dex_code.Init(this, code_start, code_end)) {
      break;
    }
...
int DexFile::GetVirtualMethod(std::string& method_name, std::string& proto_name,
  ...
  int ret = -1;
  do {
    if (class_data_item_->virtual_methods.end() == virtual_methods_itor_) {
      break;
    }
...
int DexFile::GetStringString(uint32_t index, std::string& str) {
  if (index >= dex_format_->string_ids.size()) {
    return -2;
  }
  ...
  int bytes_read = 0;
  if (0 != ReadUleb128Safe(cur, BufEnd(), &utf16_size, &bytes_read)) {
    return -2;
  }
  ...
  if (nullptr ==
      std::memchr(str_begin, 0, static_cast<size_t>(tail - str_begin))) {
    return -2;
  }
  try {
    str = str_begin;
...
int DexFile::GetTypeString(uint32_t index, std::string& str) {
  if (index >= dex_format_->type_ids.size()) {
    return -2;
  }
...
int DexFile::GetProtoInfo(uint32_t index, ProtoInfo& proto_info) {
  if (index >= dex_format_->proto_ids.size()) {
    return -2;
  }
  ...
  uint32_t size = 0;
  std::memcpy(&size, base + parameters_off, sizeof(size));
...
int DexFile::GetFieldInfo(uint32_t index, FieldInfo& field_info) {
  if (index >= dex_format_->field_ids.size()) {
    return -2;
  }
...
int DexFile::GetMethodInfo(uint32_t index, MethodInfo& method_info) {
  if (index >= dex_format_->method_ids.size()) {
    return -2;
  }
...
int DexFile::GetClassInfo(uint32_t index, ClassInfo& class_info) {
  if (index >= dex_format_->class_defs.size()) {
    return -2;
  }
...
int DexFile::ParseHeader() {
  ...
  if (target_size_ < sizeof(DexHeader)) {
    return -1;
  }
  std::memcpy(header_item, dex_file_buf_, sizeof(DexHeader));
  if (!IsSupportedDexMagic(header_item->magic)) {
    return -1;
  }
  if (header_item->file_size != target_size_) {
    return -1;
  }
  if (0x70 != header_item->header_size) {
    return -1;
  }

  if (kEndianConstant == header_item->endian_tag) {
    little_endian_tag_ = true;
  } else if (kReverseEndianConstant == header_item->endian_tag) {
...
int DexFile::ParseTypeIds() {
  ...
  if (!RegionOk(header_item->type_ids_off, size, sizeof(DexTypeId))) {
    return -1;
  }
