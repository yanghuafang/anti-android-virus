  ...
  try {
    ...
    while (cur < static_cast<uint8_t*>(code_end_)) {
      ...
      if (0x1a == opcode || 0x1b == opcode) {
        uint32_t index = 0;
        // Loaded with memcpy: cur points into the mapped DEX at an offset the
        // file chooses, so it is not necessarily 4-byte aligned and a
        // dereferenced reinterpret_cast is undefined -- the same reason
        // Blowfish::Encrypt copies its halves.
        if (0x1a == opcode) {  // const-string vAA, string@BBBB
          uint16_t narrow = 0;
          std::memcpy(&narrow, cur + sizeof(uint16_t), sizeof(narrow));
          index = narrow;
        } else {  // const-string/jumbo vAA, string@BBBBBBBB
          std::memcpy(&index, cur + sizeof(uint16_t), sizeof(index));
        }
