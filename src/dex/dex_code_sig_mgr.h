...
class DexCodeSigMgr {
 public:
  ...
  // Owns the parsed code-signature tables for one database.
  DexCodeSigMgr(const DexCodeSigMgr&) = delete;
  DexCodeSigMgr& operator=(const DexCodeSigMgr&) = delete;

  int Init(ISigMgr* sig_mgr);
  int Uninit();

  int SearchOpcodeMap(const FastOpcodes* opcodes) const;
 ...
 private:
  ...
  static int PrepareOpcodeMap(const uint8_t* map_in_file,
