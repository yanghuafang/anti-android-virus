...
int AcTree::Search(const std::vector<uint32_t>& path, DexPathSig** sig) {
  ...
  if (nullptr == root_ || root_->children.empty()) {
    return -1;
  }
  ...
  while (i < path.size()) {
    if (cur->children.empty()) {
      ...
      if (TryMatch(cur, path.size(), i, sig)) {
        return 0;
      }
      cur = cur->failure;
      if (root == cur) {
...
AcNode* AcTree::FindChild(AcNode* node, uint32_t value) {
  ...
  if (it != node->children.end() && (*it)->value == value) {
    return it->get();
  }
  return nullptr;
}
...
int AcTree::LinkFailure(AcNode* node) {
  ...
  if (0 != LinkFailureNode(node)) {
    return -1;
  }
  for (auto& child : node->children) {
    if (0 != LinkFailure(child.get())) {
      return -1;
    }
  }
  return 0;
...
int AcTree::LinkFailureNode(AcNode* node) {
  ...
  if (0 != GetSuffixPath(node, suffix_path)) {
    return -1;
  }
...
bool AcTree::IsMatchAt(size_t path_size, uint32_t index, uint32_t layer,
  ...
  if (index + 1 == layer) {
    if (layer == path_size && kStrMatchTypeEqual == match_type) {
      return true;
    }
    if (layer < path_size && kStrMatchTypeStartWith == match_type) {
      return true;
    }
  } else if (index + 1 > layer) {
    if (layer < path_size && kStrMatchTypeEndWith == match_type) {
      return true;
    }
  } else {
    assert(false);
  ...
  if (kStrMatchTypeContain == match_type) {
    return true;
  }
  return false;
}
