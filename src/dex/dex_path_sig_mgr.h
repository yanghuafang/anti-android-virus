...
class DexPathSigMgr {
 public:
  ...
  // Owns the parsed path signatures and the trie built from them.
  DexPathSigMgr(const DexPathSigMgr&) = delete;
  DexPathSigMgr& operator=(const DexPathSigMgr&) = delete;
 ...
 private:
  ...
  int CreateAcTree();
  static int CalcCrC32(const char* str, int len, uint32_t& crc);
