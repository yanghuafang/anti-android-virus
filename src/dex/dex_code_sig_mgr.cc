...
DexCodeSigMgr::DexCodeSigMgr() = default;
...
int DexCodeSigMgr::SearchOpcodeMap(const FastOpcodes* opcodes) const {
  if (nullptr == opcodes) {
    return -1;
  }

  if (!opcode_map_.map01.test(opcodes->opcode01)) {
    return -1;
  }
  if (!opcode_map_.map23.test(opcodes->opcode23)) {
    return -1;
  }
  if (!opcode_map_.map45.test(opcodes->opcode45)) {
    return -1;
  }
  if (!opcode_map_.map67.test(opcodes->opcode67)) {
    return -1;
  }
...
int DexCodeSigMgr::SearchOpcodeCrc(uint32_t crc, DexCodeCrcSig** opcode_sig) {
  if (nullptr == opcode_sig) {
    return -1;
  }
  if (opcode_crc_sig_array_.empty()) {
    return -1;
  }
  ...
  while (l <= r) {
    int m = (l + r) / 2;
    uint32_t v = opcode_crc_sig_array_[m].crc;
    if (v < crc) {
      l = m + 1;
    } else if (v > crc) {
      r = m - 1;
...
int DexCodeSigMgr::SearchOperandCrc(uint32_t crc, DexCodeCrcSig** operand_sig) {
  if (nullptr == operand_sig) {
    return -1;
  }
  if (operand_crc_sig_array_.empty()) {
    return -1;
  }
  ...
  while (l <= r) {
    int m = (l + r) / 2;
    uint32_t v = operand_crc_sig_array_[m].crc;
    if (v < crc) {
      l = m + 1;
    } else if (v > crc) {
      r = m - 1;
...
int DexCodeSigMgr::SearchCodeLogic(uint32_t sig_id,
  ...
  if (nullptr == logic_sig) {
    return -1;
  }
  if (code_logic_sig_array_.empty()) {
    return -1;
  }
  ...
  while (l <= r) {
    int m = (l + r) / 2;
    uint32_t v = code_logic_sig_array_[m].sig_id;
    if (v < sig_id) {
      l = m + 1;
    } else if (v > sig_id) {
      r = m - 1;
...
int DexCodeSigMgr::ParseOpcodeMap(const SigItem* opcode_map_item) {
  ...
  if (nullptr == opcode_map_item) {
    return -1;
  }
  if (kSigFormatDexOpcodeMap != opcode_map_item->format) {
    return -1;
  }
  if ((kFastOpcodesCount / 2) != opcode_map_item->sig_count) {
    return -1;
  }
  if (nullptr == opcode_map_item->buf) {
    return -1;
  }
  ...
  if (0 != PrepareOpcodeMap(map01, opcode_map_.map01)) {
    return -1;
  }
  if (0 != PrepareOpcodeMap(map23, opcode_map_.map23)) {
    return -1;
  }
  if (0 != PrepareOpcodeMap(map45, opcode_map_.map45)) {
    return -1;
  }
  if (0 != PrepareOpcodeMap(map67, opcode_map_.map67)) {
    return -1;
  }
...
int DexCodeSigMgr::ParseOpcodeCrcSig(const SigItem* opcode_crc_sig_item) {
  ...
  if (nullptr == opcode_crc_sig_item) {
    return -1;
  }
  if (kSigFormatDexOpcodeCrc != opcode_crc_sig_item->format) {
    return -1;
  }
  ...
  if (nullptr == opcode_crc_sig_item->buf) {
    return -1;
  }
...
int DexCodeSigMgr::ParseOperandCrcSig(const SigItem* operand_crc_sig_item) {
  ...
  if (nullptr == operand_crc_sig_item) {
    return -1;
  }
  if (kSigFormatDexOperandCrc != operand_crc_sig_item->format) {
    return -1;
  }
  ...
  if (nullptr == operand_crc_sig_item->buf) {
    return -1;
  }
...
int DexCodeSigMgr::ParseCodeLogicSig(const SigItem* code_logic_sig_item) {
  ...
  if (nullptr == code_logic_sig_item) {
    return -1;
  }
  if (kSigFormatDexCodeLogic != code_logic_sig_item->format) {
    return -1;
  }
  ...
  if (nullptr == code_logic_sig_item->buf) {
    return -1;
  }
  ...
  auto read_block = [&](std::vector<uint32_t>& out) -> bool {
    if (buf_end - cur < static_cast<ptrdiff_t>(sizeof(uint32_t))) {
      return false;
    }
  ...
  try {
    ...
    for (uint32_t i = 0; i < code_logic_sig_item->sig_count; i++) {
      ...
      if (!read_block(logic_sig.not_crcs)) {
        return -1;
      }
      if (!read_block(logic_sig.xor_crcs)) {
        return -1;
      }
      if (!read_block(logic_sig.and_crcs)) {
        return -1;
      }
      if (!read_block(logic_sig.or_crcs)) {
        return -1;
      }
...
int DexCodeSigMgr::PrepareOpcodeMap(const uint8_t* map_in_file,
  ...
  for (int i = 0; i < kBitMapSize / (8 * 4); i++) {
    uint32_t base = 8 * 4 * i;
    uint32_t value = Rd32(reinterpret_cast<const char*>(map_in_file) +
                          (i * sizeof(uint32_t)));
    uint32_t mask = 0x01;
    for (int j = 0; j < 8 * 4; j++) {
      if (0 != (value & mask)) {
