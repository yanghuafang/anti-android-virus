#include "dex/dex_path_scan_result.h"
...
uint32_t DexPathScanResult::SigId() const { return sig_id_; }
...
bool DexPathScanResult::IsMalware() const {
  if (not_match_count_ > 0) {
    return false;
  }
  if (xor_match_count_ > 1) {
    return false;
  }

  if (or_match_count_ > 0) {
    return true;
  }
  if (and_match_count_ > 1) {
    return true;
  }
  if (1 == xor_match_count_) {
    return true;
  }
