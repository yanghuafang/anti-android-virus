...
int DexScanner::Init(void* context) {
  ...
  if (nullptr == dex_sig_mgr_) {
    return -1;
  }
  if (0 != dex_sig_mgr_->Init(sig_mgr)) {
    return -1;
  }
  return 0;
}
...
int DexScanner::ScanTarget(ITarget* target, const ScanOption* option,
                           ScanResultPtr& result) {
  if (nullptr == target || nullptr == option) {
    return -1;
  }
  if (!option->config.dex) {
    return -1;
  }
  ...
  if (nullptr == dex_parser) {
    return -1;
  }
  if (0 != dex_parser->Init(dex_sig_mgr_.get(), target)) {
    return -1;
  }

  std::vector<uint32_t> sig_id_array;
  if (0 != dex_parser->Scan(sig_id_array)) {
    return -1;
  }
  ...
  if (!sig_id_array.empty()) {
    ...
    ScanResult* scan_result = static_cast<ScanResult*>(std::malloc(size));
    if (nullptr == scan_result) {
      return -1;
    }
    ...
    ScanResult* scan_result =
        static_cast<ScanResult*>(std::malloc(sizeof(ScanResult)));
    if (nullptr == scan_result) {
      return -1;
    }
    scan_result->is_white = 0;
