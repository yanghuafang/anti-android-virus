...
void OnReport(const aav::ScanReport* report, void* user_data) {
  ...
  for (size_t i = 0; i < report->sig_count; i++) {
    if (report->sig_ids[i] == 1001) {
      c->has_1001 = true;
    }
    if (report->sig_ids[i] == 1002) {
      c->has_1002 = true;
    }
  }
}
...
int CheckBuffer(aav::IEngine* engine, const std::string& bytes,
  ...
  if (0 !=
    ...
    std::printf("FAIL: ScanBuffer(%s) returned an error\n", label);
    return 1;
  }
  if (!c.malware || !c.has_1001 || !c.has_1002) {
    std::printf("FAIL: %s not detected (malware=%d 1001=%d 1002=%d)\n", label,
                c.malware, c.has_1001, c.has_1002);
    return 1;
  }
  std::printf("OK: %s detected\n", label);
  return 0;
}
...
int main(int argc, char** argv) {
  if (argc < 4) {
    std::printf("usage: %s <signature-db> <sample.dex> <sample.apk>\n",
                argv[0]);
    return 2;
  }
  ...
  if (dex.empty() || apk.empty()) {
    std::printf("FAIL: could not read sample.dex / sample.apk\n");
    rc = 2;
  } else {
