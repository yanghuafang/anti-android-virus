...
using aav::FileSource;
using aav::IFileStream;
using aav::IFileTarget;
using aav::IMemStream;
using aav::IMemTarget;
using aav::IModule;
using aav::MakeFileStream;
using aav::MakeFileTarget;
using aav::MakeMemStream;
using aav::MakeMemTarget;
using aav::MakeModule;
using aav::MemSource;
using aav::ObjPtr;
...
TEST_CASE("MemStream reads, seeks, tells, and exposes a view") {
  ...
  CHECK(view_size == 256);
  CHECK(std::memcmp(view, data.data(), 256) == 0);
}
...
TEST_CASE("FileTarget maps a file and exposes its bytes") {
  ...
  CHECK(t->GetBuf(&buf) == 0);
  REQUIRE(buf != nullptr);
  CHECK(std::memcmp(buf, data.data(), data.size()) == 0);
}
...
TEST_CASE("FileStream reads and memory-maps a file") {
  ...
  CHECK(std::memcmp(view, data.data(), data.size()) == 0);
}
